...
description:
  ...
  self-healing CI fixes. ALWAYS USE THIS SKILL instead of native CI provider
  tools (gh, glab, etc.) for CI monitoring.
---
...
1. **Parse throttle message** for CIPE URLs using regex matching `/cipes/{id}`
   pattern (format: `https://...nx.app/cipes/{cipeId}/self-healing`)
2. **Reject previous fixes** — for each CIPE URL found:
   - Call `ci_information({ url: "<cipe_url>" })` to get the `shortLink`
   - Call
     `update_self_healing_fix({ shortLink: "<shortLink>", action: "REJECT" })`
     to reject
3. **Attempt local fix**:
...
5. After rejecting fixes and pushing, self-healing should resume since throttle
   condition (unapplied fixes) is cleared
